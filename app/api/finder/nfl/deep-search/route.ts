...
import { runNFLFinderDeepSearch } from '@/lib/finder/nflEngine';
...
export async function GET(request: Request) {
  ...
  // Same validation/dev-only escape hatch as the MLB route: never affects the normal cache path.
  ...
  if (validationMaxGames != null || validationMaxCredits != null || preflight) {
    try {
      const data = await runNFLFinderDeepSearch(slateDate, { validationMaxGames, validationMaxCredits, preflight });
  ...
  try {
    ...
    const data = await runNFLFinderDeepSearch(slateDate);
